	...
	t.Run("should be comparable", func(t *testing.T) {
		var (
			c1 domain.ProviderConfig = docker.Data{
				Host: monad.Value[docker.Host]("testdata"),
	...
	t.Run("should correctly parse a host", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.value, func(t *testing.T) {
				got, err := docker.HostFrom(tt.value)

				if !tt.valid {
					testutil.ErrorIs(t, docker.ErrInvalidHost, err)
					return
				}
...
-----END OPENSSH PRIVATE KEY-----`, true},
		...
		for _, tt := range tests {
			t.Run(tt.value, func(t *testing.T) {
				got, err := docker.PrivateKeyFrom(tt.value)

				if !tt.valid {
					testutil.ErrorIs(t, docker.ErrInvalidSSHKey, err)
					return
				}
