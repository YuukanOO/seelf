...
type (
	...
	Scheduler interface {
		...
		Queue(context.Context, Request, monad.Maybe[string]) error
	}
	...
	SchedulerAdapter interface {
		...
		Create(context.Context, Request, monad.Maybe[string]) error // Create a new scheduled job
...
func (s *DefaultScheduler) Queue(
	...
	return s.adapter.Create(ctx, msg, dedupeName)
}
