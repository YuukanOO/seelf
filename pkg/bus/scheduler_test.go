package bus_test

import (
	"context"
	"errors"
	"slices"
	"strconv"
	"sync"
	"testing"

	"github.com/YuukanOO/seelf/pkg/bus"
	"github.com/YuukanOO/seelf/pkg/bus/memory"
	"github.com/YuukanOO/seelf/pkg/log"
	"github.com/YuukanOO/seelf/pkg/must"
	"github.com/YuukanOO/seelf/pkg/testutil"
)

func TestScheduler(t *testing.T) {
	logger := must.Panic(log.NewLogger())
	b := memory.NewBus()
	// Register an handler which will just return the inner cmd error to test how the scheduler behave.
	bus.Register(b, func(_ context.Context, cmd returnCommand) (bus.UnitType, error) {
		return bus.Unit, cmd.err
	})

	t.Run("should queue and handle the job return appropriately", func(t *testing.T) {
		adapter := &adapter{}
		scheduler := bus.NewScheduler(adapter, logger, b, 0, bus.WorkerGroup{
			Size:     4,
			Messages: []string{returnCommand{}.Name_()},
		})

		scheduler.Start()
		defer scheduler.Stop()

		innerErr := errors.New("some error")

		withoutErr := returnCommand{}
		withUnwrapedErr := returnCommand{err: innerErr}
		withIgnoredErr := returnCommand{err: bus.Ignore(innerErr)}
		withPreservedOrderErr := returnCommand{err: bus.PreserveOrder(innerErr)}

		testutil.IsNil(t, scheduler.Queue(context.Background(), withoutErr))
		testutil.IsNil(t, scheduler.Queue(context.Background(), withUnwrapedErr))
		testutil.IsNil(t, scheduler.Queue(context.Background(), withIgnoredErr))
		testutil.IsNil(t, scheduler.Queue(context.Background(), withPreservedOrderErr))
		testutil.IsNil(t, scheduler.Queue(context.Background(), addCommand{}))

		adapter.wait()

		testutil.HasLength(t, adapter.done, 2)
		slices.SortFunc(adapter.done, func(a, b *job) int {
			return a.id - b.id
		})

		testutil.Equals(t, 0, adapter.done[0].id)
		testutil.Equals(t, 2, adapter.done[1].id)

		testutil.HasLength(t, adapter.retried, 3)
		slices.SortFunc(adapter.retried, func(a, b *job) int {
			return a.id - b.id
		})

		testutil.Equals(t, 1, adapter.retried[0].id)
		testutil.ErrorIs(t, innerErr, adapter.retried[0].err)
		testutil.IsFalse(t, adapter.retried[0].preserveOrder)

		testutil.Equals(t, 3, adapter.retried[1].id)
		testutil.ErrorIs(t, innerErr, adapter.retried[1].err)
		testutil.IsTrue(t, adapter.retried[1].preserveOrder)

		testutil.Equals(t, 4, adapter.retried[2].id)
		testutil.ErrorIs(t, bus.ErrNoHandlerRegistered, adapter.retried[2].err)
	})
}

var (
	_ bus.ScheduledJob     = (*job)(nil)
	_ bus.SchedulerAdapter = (*adapter)(nil)
	_ bus.Request          = (*returnCommand)(nil)
)

type (
	job struct {
		id            int
		msg           bus.Request
		err           error
		preserveOrder bool
	}

	adapter struct {
		wg      sync.WaitGroup
		jobs    []*job
		done    []*job
		retried []*job
	}

	returnCommand struct {
		bus.Command[bus.UnitType]

		err error
	}
)

func (r returnCommand) Name_() string { return "returnCommand" }

func (j *job) ID() string           { return strconv.Itoa(j.id) }
func (j *job) Message() bus.Request { return j.msg }

func (a *adapter) Setup() error { return nil }

func (a *adapter) Create(_ context.Context, msg bus.Request, _ bus.JobOptions) error {
	a.wg.Add(1)
	a.jobs = append(a.jobs, &job{id: len(a.jobs), msg: msg})
	return nil
}

func (a *adapter) wait() {
	a.wg.Wait()
}

func (a *adapter) GetNextPendingJobs(context.Context) ([]bus.ScheduledJob, error) {
	j := make([]bus.ScheduledJob, len(a.jobs))

	for i, job := range a.jobs {
		j[i] = job
	}

	a.jobs = nil

	return j, nil
}

func (a *adapter) Retry(_ context.Context, j bus.ScheduledJob, jobErr error, preserveOrder bool) error {
	defer a.wg.Done()
	jo := j.(*job)
	jo.err = jobErr
	jo.preserveOrder = preserveOrder

	a.retried = append(a.retried, jo)
	return nil

}

func (a *adapter) Done(_ context.Context, j bus.ScheduledJob) error {
	defer a.wg.Done()
	a.done = append(a.done, j.(*job))
	return nil
}
