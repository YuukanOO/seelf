...
type (
	...
	Dataloader[T any] interface {
		...
		Fetch(Executor, context.Context, storage.KeyedResult[T]) error // Fetch related data represented by this dataloader.
	}

	dataLoader[T any] struct {
		extractor func(T) string
		fetcher   func(Executor, context.Context, storage.KeyedResult[T]) error
	}
)
...
func (l *dataLoader[T]) Fetch(ex Executor, ctx context.Context, result storage.KeyedResult[T]) error {
